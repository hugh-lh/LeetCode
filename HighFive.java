...
public class HighFive {
	public Map<Integer, Double> getHighFive(Result[] resultArray) {
		...
		if (resultArray == null || resultArray.length == 0) return res;
		Map<Integer, PriorityQueue<Integer>> scoreRecord = new HashMap<>();
		for (Result s : resultArray) {
			scoreRecord.putIfAbsent(s.id, new PriorityQueue<Integer>());
			PriorityQueue<Integer> temp = scoreRecord.get(s.id);
			temp.offer(s.value);
			if (temp.size() > 5) temp.poll();			
		}
		for (int id : scoreRecord.keySet()) {
			PriorityQueue<Integer> temp = scoreRecord.get(id);
			double sum = 0;
			for (int score : temp) sum += score;
			res.put(id, sum/temp.size());
		}
		return res;
	}
